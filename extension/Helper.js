class Helper {
   ...
   * @returns {Array}
   */
  static extract (array, key) {
    const values = []
    let counter = array.length
    while (counter--) {
      if (array[counter][key] === undefined) continue
      values.push(array[counter][key])
    }
    return values
  }

  static addcslashes (str, charlist) {
  ...
  static mapFiltersToQueryBuilder (inputFilters, queryBuilder, categoryPath = '') {
    ...
    for (const key in inputFilters) {
      ...
      if (key === 'display_amount') { // we have price filter (the only one with a range)
        queryBuilder.setPriceRange(inputFilters[key].minimum, inputFilters[key].maximum)
      } else if (values) {
        filters[key] = values
      }
    }
